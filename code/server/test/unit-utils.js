...
*              User
*****************************************************************************************************/
exports.testCreateUser = function (itShould, newUser, expectedRejectionCode = undefined) {
	test(`Should ${itShould}`, async () => {
		if (!expectedRejectionCode) {
			const res = await UserManager.createUser(newUser);
			const createdUser = await PersistentManager.loadAll(User.tableName).then(users => users[0]);

			expect(res).toEqual(newUser.userId);
			expect(createdUser).toEqual(newUser);
		}
		else {
			await expect(UserManager.createUser(newUser)).rejects.toHaveProperty("code", expectedRejectionCode);
		}
	});
}
...
exports.testUpdateUser = function (itShould, newUser, attributeName, value, expectedRejectionCode = undefined) {
	test(`Should ${itShould}`, async () => {
		if (!expectedRejectionCode) {
			...
			let updatedUsers = await PersistentManager.loadAllByAttribute(User.tableName, attributeName, value);
...
exports.testDeleteUser = function (itShould, attributeName, value) {
	test(`Should ${itShould}`, async () => {
		await UserManager.deleteUser(attributeName, value);
		const numOfRemainingUsers = await PersistentManager.loadAllByAttribute(User.tableName, attributeName, value).then(users => users.length);

		expect(numOfRemainingUsers).toEqual(0);
	})
}

exports.testDeleteAllUser = function (itShould) {
	test(`Should ${itShould}`, async () => {
		await UserManager.deleteAllUser();
		const numOfRemainingUsers = await PersistentManager.loadAll(User.tableName).then(users => users.length);
...
exports.testLoadAllUser = function (itShould, expectedLength) {
	test(`Should ${itShould}`, async () => {
		const actualLength = await UserManager.loadAllUser().then(users => users.length);
...
exports.testLoadAllByAttributeUser = function (itShould, attributeName, value) {
	test(`Should ${itShould}`, async () => {
		const res = await UserManager.testLoadAllByAttributeUser(attributeName, value).then(users => users.length);
...
exports.testGetAllUsers = function (itShould, expectedLength, expectedGetAllUsersProperties) {
	test(`Should ${itShould}`, async () => {
		const res = await UserManager.getAllUsers();

		expect(res.length).toEqual(expectedLength);
		for (const r of res) {
			for (const p of expectedGetAllUsersProperties) {
				expect(r).toHaveProperty(p);
			}
		}
	})
...
exports.testGetUserByUserId = function (itShould, userId, expectedGetUserByIdProperties = undefined, expectedRejectionCode = undefined) {
	test(`Should ${itShould}`, async () => {
		if (!expectedRejectionCode) {
			const res = await UserManager.getUserById(userId);

			for (const p of expectedGetUserByIdProperties) {
				expect(res).toHaveProperty(p);
			}
		} else {
			await expect(UserManager.getUserById(userId)).rejects.toHaveProperty("code", expectedRejectionCode);
		}
	})
}

exports.testGetUserByEmail = function (itShould, email, expectedGetUserByEmailProperties = undefined, expectedRejectionCode = undefined) {
	test(`Should ${itShould}`, async () => {
		if (!expectedRejectionCode) {
			const res = await UserManager.getUserByEmail(email);

			for (const p of expectedGetUserByEmailProperties) {
				expect(res).toHaveProperty(p);
			}
		} else {
			await expect(UserManager.getUserByEmail(email)).rejects.toHaveProperty("code", expectedRejectionCode);
		}
	})
}
